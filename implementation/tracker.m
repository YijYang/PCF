...
params.learning_rate = params.learning_rate_pcf1;
...
kx = cellfun(@(sz) -ceil((sz(1) - 1)/2) : 0, filter_sz_cell, 'uniformoutput', false);

% construct the Uniform label function
...
prior_weights_pcf = zeros(params.nSamples,1, 'single');
...
while true
    ...
    if seq.frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            scores_fs_feat{k1} = sum(bsxfun(@times, hf_full_pcf{k1}, xtf_proj{k1}), 3);
            scores_fs_sum_pcf = scores_fs_feat{k1};
            for k = block_inds
                scores_fs_feat{k} = sum(bsxfun(@times, hf_full_pcf{k}, xtf_proj{k}), 3);
                scores_fs_sum_pcf(1+pad_sz{k}(1):end-pad_sz{k}(1), 1+pad_sz{k}(2):end-pad_sz{k}(2),1,:) = ...
                    scores_fs_sum_pcf(1+pad_sz{k}(1):end-pad_sz{k}(1), 1+pad_sz{k}(2):end-pad_sz{k}(2),1,:) + ...
                    scores_fs_feat{k};
            end
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% The response of the poistion filters
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%          
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% The response of the scale filter
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%              
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
    ...
    if params.use_sample_merge
        ...
        % Update the different weights of the samples
            params.learning_rate = params.learning_rate_pcf2;
            [merged_sample, new_sample, merged_sample_id, new_sample_id, distance_matrix, gram_matrix, prior_weights_pcf] = ...
                update_sample_space_model_pcf(samplesf, xlf_proj_perm, distance_matrix, gram_matrix, prior_weights_pcf,...
                num_training_samples,params);
            params.learning_rate = params.learning_rate_pcf1;
            [merged_sample, new_sample, merged_sample_id, new_sample_id, distance_matrix, gram_matrix, prior_weights] = ...
                update_sample_space_model_pcf(samplesf, xlf_proj_perm, distance_matrix, gram_matrix, prior_weights,...
                num_training_samples,params);
        end
    ...
    sample_weights = cast(prior_weights, 'like', params.data_type);
    sample_weights_pcf = cast(prior_weights_pcf, 'like', params.data_type);
    ...
    if train_tracker     
        ...
        if seq.frame == 1 && params.update_projection_matrix
            ...
            % Init the position correlation filters
            ...
            % Update the PCF
                [hf_pcf, res_norms, CG_state] = train_filter(hf_pcf, samplesf, yf, reg_filter, sample_weights_pcf, sample_energy, reg_energy, params, CG_opts, CG_state);              
        ...
        hf_full_pcf = full_fourier_coeff(hf_pcf);
